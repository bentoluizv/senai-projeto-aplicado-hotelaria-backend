...
class AccommodationDAO:
    ...
    def insert(self, accommodation) -> None:
        cursor = self.db.cursor()
        cursor.execute(
            "INSERT INTO accommodation (uuid, created_at, name, status, total_guests, single_beds, double_beds, min_nights, price) VALUES (:uuid, :created_at, :name, :status, :total_guests, :single_beds, :double_beds, :min_nights, :price);",
            accommodation,
        )
        ...
        for amenitie in accommodation["amenities"]:
            ...
            cursor.execute(
                "INSERT INTO amenities_per_accommodation (accommodation_uuid, amenitie_id) VALUES (?, ?)",
                (accommodation["uuid"], amenitie_id),
            )
    ...
    def update(self, uuid: str, accommodation) -> None:
        ...
        cursor.execute(
            "DELETE FROM amenities_per_accommodation WHERE accommodation_uuid = ?",
            (uuid,),
        )
