...
class BookingDAO:
    ...
    def count(self):
        ...
        count = cursor.execute('SELECT COUNT(*) FROM booking').fetchone().get('COUNT(*)')
        return count
    ...
    def insert(self, booking: BookingDTO) -> None:
        ...
        cursor = self.db.cursor()
        cursor.execute(statement, (booking['uuid'], booking['created_at'], booking['status'], booking['check_in'], booking['check_out'], booking['guest_document'], booking['accommodation_uuid']))
        self.db.commit()


    def findBy(self, property: str, value: str):
        cursor = self.db.cursor()
        cursor.execute(f'SELECT uuid, status, created_at, check_in, check_out, document, accommodation_uuid FROM booking WHERE booking.{property} = ?;',  (value,))
        ...
        cursor.execute('SELECT created_at, document, name, surname, country, phone FROM guest WHERE guest.document = ?', (guest_document,))
        guest = cursor.fetchone()

        cursor.execute('SELECT uuid, created_at, name, status, total_guests, single_beds, double_beds, min_nights, price FROM accommodation WHERE accommodation.uuid = ?', (accommodation_uuid,))
    ...
    def update(self, uuid: str, booking) -> None:
        ...
        cursor = self.db.cursor()
        cursor.execute(statement, (booking
        ['status'], booking['check_in'],  booking['check_out'],  booking['guest_document'],  booking['accommodation_uuid'],  uuid))
        self.db.commit()
